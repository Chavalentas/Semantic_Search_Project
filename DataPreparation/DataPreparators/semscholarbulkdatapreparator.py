...
class SemanticScholarBulkDataPreparator(DataPreparator):
    """Represents a data preparator the prepare data fetched from the Semantic Scholar API.
    ...
    def __init__(self, amount_to_extract: int, exclude_empty_titles: bool, exclude_empty_abstracts: bool):
        """Initializes a new instance of SemanticScholarBulkDataPreparator.
    ...
    def prepare_data(self, data_dict: dict) -> dict:
        ...
        result_list = data_dict_c["semanticScholarPapers"]
        ...
        for el in result_list:
            if el["publicationDate"] is None:
                continue
            el["publicationDate"] = datetime.datetime.strptime(el["publicationDate"], "%Y-%m-%d")

        data_dict_c["semanticScholarPapers"] = result_list[:self.__amount_to_extract]
        return data_dict_c
    
    def __remove_duplicate_ids(self, papers: list[dict]):
